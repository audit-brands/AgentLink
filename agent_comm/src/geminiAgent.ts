...
class GeminiAgent {
    ...
    private async runGeminiCli(prompt: string): Promise<string | JsonRpcResponse> {
        try {
            const cliPath = path.resolve(__dirname, '../gemini-cli');
            ...
            console.error('[ERROR] Gemini CLI execution failed:', error);
            return {
                jsonrpc: "2.0",
                error: {
                    code: -32000,
    ...
    private async handleJsonRpcRequest(req: Request, res: Response) {
        ...
        try {
            ...
            if (requestData.method === "RequestRefactor") {
                ...
                const output = await this.runGeminiCli(params.instruction);
    ...
    public start() {
        this.app.listen(this.PORT, () => {
            console.log(`Gemini agent running on http://localhost:${this.PORT}`);
